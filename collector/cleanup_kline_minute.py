#!/usr/bin/env python3
"""Apply trading-day retention cleanup for public.kline_minute."""
        ...
        rows = psql_query(
            ...
            WITH deleted AS (
                ...
                WHERE ctid IN (
                    ...
                    FROM public.kline_minute
                    WHERE {predicate}
                    ORDER BY trade_date NULLS FIRST, bar_time
                    LIMIT {batch_size}
                )
                RETURNING 1
            )
...
def main() -> int:
    ...
    add_pg_args(parser)
    parser.add_argument("--retention-trading-days", type=int, default=45)
